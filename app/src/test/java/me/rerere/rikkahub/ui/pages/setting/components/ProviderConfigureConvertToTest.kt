...
class ProviderConfigureConvertToTest {
    ...
    @Test
    fun `defaultBaseUrlForReset returns Codex endpoint for subscription mode`() {
        val subscription = ProviderSetting.OpenAI(
            authType = OpenAIAuthType.CHATGPT_SUBSCRIPTION,
            baseUrl = OPENAI_CODEX_BASE_URL,
        )

        assertEquals(OPENAI_CODEX_BASE_URL, subscription.defaultBaseUrlForReset())
    }
