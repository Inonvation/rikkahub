 ...
 *
 * 模式层面的控制由 ChatMode 的 DEVICE_TOOLS 能力负责，不依赖助手 localTools，
 * 保证管理模式（CREATIVE）下 AI 一定能拿到设备工具。写工具（冻结、清理）执行时
 * 经过 SafetyGuard 校验、DeviceToolPermission 三档审批，并记录到审计。
 */
class DeviceTools(
    context: Context,
    ...
    private val storageCleanerTools = buildStorageCleanerTools(permission, auditStore)
    private val freezeAppTools = buildFreezeAppTools(safetyGuard, permission, auditStore)
