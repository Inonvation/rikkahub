        ...
        enabledTools.forEach { toolName ->
            when (toolName) {
                ...
                "save_wrong_question" -> add(
                    createWrongQuestionTool(conversationId, wrongQuestionDao, studySubject.ifBlank { "math" })
                )
                "save_knowledge_card" -> add(
                    createKnowledgeCardTool(conversationId, knowledgeCardDao, studySubject.ifBlank { "politics" })
                )
                "save_note" -> add(
                    createNoteTool(conversationId, assistantId, noteDao)
                )
                "quiz_user" -> add(
